...
/**
 * This class provides simple utility methods
 * for reading and parsing program arguments from different sources.
 */
@SuppressWarnings({"unchecked", "rawtypes", "ResultOfMethodCallIgnored", "MethodDoesntCallSuperMethod", "RedundantThrows", "GrazieInspection"})
public class ParameterTool implements Serializable, Cloneable {
  ...
  // ------------------ Constructors ------------------------
  public static ParameterTool fromCmdArgs(String[] args) {
    ParameterTool parameterTool;
    if (args.length > 0 && args[0].contains("=")) {
      parameterTool = ParameterTool.fromPropertiesArgs(args);
    } else if (args.length > 0 && args[0].contains(":")) {
      parameterTool = ParameterTool.fromMapArgs(args);
    } else {
  ...
  public static Map<String, String> mapFromArgs(String[] args) {
    if (args.length > 0 && args[0].contains("=")) {
      return ParameterTool.mapFromArgs(args, "=");
    } else if (args.length > 0 && args[0].contains(":")) {
      return ParameterTool.mapFromArgs(args, ":");
    } else {
      return ParameterTool.mapFromArgs(args);
   ...
   * The arguments are keys followed by values.
  ...
  public static ParameterTool fromMapArgs(String[] args) {
    return fromArgs(args, ":");
  }

  public static ParameterTool fromPropertiesArgs(String[] args) {
    return fromArgs(args, "=");
  }

  public static ParameterTool fromArgs(String[] args, String sep) {
    return fromMap(mapFromArgs(args, sep));
  }

  public static Map<String, String> mapFromArgs(String[] args, String sep) {
    final Map<String, String> map = new HashMap<>();

    for (String arg : args) {
      if (!arg.contains(sep)) {
        throw new IllegalArgumentException(String.format("Error parsing arguments '%s' on '%s'. Please make sure args with = in them.", Arrays.toString(args), arg));
      }
      String[] splits = arg.split(sep, -1);
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(data, that.data) && Objects.equals(defaultData, that.defaultData) && Objects.equals(unrequestedParameters, that.unrequestedParameters);
  }
  ...
  /**
   * Returns the Integer value for the given key.
   * If the key does not exist it will return the default value given.
   ...
   *
   * @param pathToFile Location of the default properties file.
   * @param overwrite  Boolean flag indicating whether to overwrite the file
  ...
  public void createPropertiesFile(String pathToFile, boolean overwrite) throws IOException {
    ...
    try (final OutputStream out = new FileOutputStream(file)) {
      defaultProps.store(out, "Default file created by SparkX");
    }
  }
