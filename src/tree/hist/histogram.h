...
namespace xgboost {
namespace tree {
template <typename ExpandEntry, typename T = float, typename H = double>
class HistogramBuilder {
  /*! \brief culmulative histogram of gradients. */
  common::HistCollection<H> hist_;
  /*! \brief culmulative local parent histogram of gradients. */
  common::HistCollection<H> hist_local_worker_;
  common::GHistBuilder builder_;
  common::ParallelGHistBuilder<H> buffer_;
  BatchParam param_;
 ...
 public:
  ...
  void AddHistRows(int *starting_index, int *sync_count,
                   std::vector<ExpandEntry> const &nodes_for_explicit_hist_build,
                   std::vector<ExpandEntry> const &nodes_for_subtraction_trick, RegTree *p_tree) {
    if (is_distributed_) {
  ...
  void BuildHist(size_t page_id, common::BlockedSpace2d space, GHistIndexMatrix const &gidx,
                 ...
                 std::vector<GradientPairT<T>> const &gpair, bool force_read_by_column = false) {
    ...
    // test histogram
    /*auto nid = nodes_for_explicit_hist_build[12].nid;
    if (nid == 83 || nid == 84) {
      cout << endl;
 ...
 public:
  /* Getters for tests. */
  common::HistCollection<H> const &Histogram() { return hist_; }
    ...
    common::ParallelFor2d(space, this->n_threads_, [&](size_t node, common::Range1d r) {
      ...
      if (!((*p_tree)[entry.nid].IsLeftChild())) {
        ...
        if (!(*p_tree)[entry.nid].IsRoot()) {
          ...
          common::SubtractionHist(this_hist, parent_hist, sibling_hist, r.begin(), r.end());
        }
      }
    });
  }
