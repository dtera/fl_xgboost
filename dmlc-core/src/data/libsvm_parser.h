...
class LibSVMParser : public TextParserBase<IndexType> {
 public:
  ...
  explicit LibSVMParser(InputSplit *source, const std::map<std::string, std::string> &args,
                        int nthread)
...
void LibSVMParser<IndexType, DType>::ParseBlock(const char *begin, const char *end,
                                                RowBlockContainer<IndexType, DType> *out) {
  out->Clear();
  ...
  while (lbegin != end) {
    ...
    if (p != lend && (*p == '@')) {
      ++p;
      sid = static_cast<uint64_t>(atoll(p));
    ...
    if (r != 2 || out->sid.empty()) {
      out->label.push_back(label < 0 ? 0 : label);
    } else {
      out->label.push_back(-1);
    }
    // parse qid:id
    uint64_t qid;
    p = q;
