...
class PheBenchmarks {
 public:
  void SetupAndRegister(phe::SchemaType schema_type, int key_size) {
    ...
    benchmark::RegisterBenchmark(fmt::format("{:^9}|Encrypt", he_kit_->GetSchemaType()).c_str(),
                                 [this](benchmark::State& st) { Encrypt(st); })
        ->Unit(benchmark::kMillisecond);
    benchmark::RegisterBenchmark(fmt::format("{:^9}|CT+CT", he_kit_->GetSchemaType()).c_str(),
                                 [this](benchmark::State& st) { AddCipher(st); })
        ->Unit(benchmark::kMillisecond);
    benchmark::RegisterBenchmark(fmt::format("{:^9}|CT-CT", he_kit_->GetSchemaType()).c_str(),
                                 [this](benchmark::State& st) { SubCipher(st); })
        ->Unit(benchmark::kMillisecond);
    benchmark::RegisterBenchmark(fmt::format("{:^9}|CT+PT", he_kit_->GetSchemaType()).c_str(),
    ...
    benchmark::RegisterBenchmark(fmt::format("{:^9}|CT*PT", he_kit_->GetSchemaType()).c_str(),
        ...
        ->Unit(benchmark::kMillisecond);
    benchmark::RegisterBenchmark(fmt::format("{:^9}|Decrypt", he_kit_->GetSchemaType()).c_str(),
...
int main(int argc, char** argv) {
  ...
  benchmark::AddCustomContext("Run times", fmt::format("{}", heu::lib::bench::kTestSize));
